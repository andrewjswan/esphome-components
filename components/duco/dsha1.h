...
class DSHA1 {
 ...
 private:
  ...
  uint32_t static inline readBE32(const unsigned char *ptr) { return __builtin_bswap32(*(uint32_t *) ptr); }

  void static inline writeBE32(unsigned char *ptr, uint32_t x) { *(uint32_t *) ptr = __builtin_bswap32(x); }

  void static inline writeBE64(unsigned char *ptr, uint64_t x) { *(uint64_t *) ptr = __builtin_bswap64(x); }
};
