...
struct CRGB {
  ...
  inline CRGB(const CHSV &rhs) __attribute__((always_inline)) { hsv2rgb_rainbow(rhs.h << 8, rhs.s, rhs.v, raw, false); }
  ...
  // fadeLightBy is a synonym for nscale8x3_video(), as a light instead of a scale
  inline CRGB &fadeLightBy(uint8_t fadefactor) {
    uint32_t scale_fixed = 256 - fadefactor;
    r = (r == 0) ? 0 : (((((uint32_t) r) * scale_fixed) >> 8) ? ((((uint32_t) r) * scale_fixed) >> 8) : 1);
    g = (g == 0) ? 0 : (((((uint32_t) g) * scale_fixed) >> 8) ? ((((uint32_t) g) * scale_fixed) >> 8) : 1);
    b = (b == 0) ? 0 : (((((uint32_t) b) * scale_fixed) >> 8) ? ((((uint32_t) b) * scale_fixed) >> 8) : 1);
    return *this;
  }
  ...
  inline explicit operator uint32_t() const { return (uint32_t{r} << 16) | (uint32_t{g} << 8) | uint32_t{b}; }
...
__attribute__((always_inline)) inline bool operator!=(const CRGB &lhs, const CRGB &rhs) { return !(lhs == rhs); }
...
class CRGBPalette16 {
 public:
  ...
  // Copy constructor
  CRGBPalette16(const CRGBPalette16 &rhs) { memmove((void *) &(entries[0]), &(rhs.entries[0]), sizeof(entries)); }

  // Create palette from array of CRGB colors
  CRGBPalette16(const CRGB rhs[16]) { memmove((void *) &(entries[0]), &(rhs[0]), sizeof(entries)); }
  ...
  CRGBPalette16(const CRGB &c1) { fill_solid_RGB(&(entries[0]), 16, c1); }
