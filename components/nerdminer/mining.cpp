...
void runStratumWorker(void *name) {
  ...
  #ifdef DEBUG_MEMORY
  ESP_LOGD(TAG, "### [Total Heap / Free heap / Min free heap]: %d / %d / %d", ESP.getHeapSize(), ESP.getFreeHeap(), ESP.getMinFreeHeap());
  #endif
  ...
  while(true) {
    ...
    while(client.connected() && client.available()) {
      ...
      switch (result) {
          case STRATUM_PARSE_ERROR:   ESP_LOGD(TAG, "  Parsed JSON: error on JSON"); break;
          ...
          case MINING_SET_DIFFICULTY: parse_mining_set_difficulty(line, currentPoolDifficulty);
                                      mMiner.poolDifficulty = currentPoolDifficulty;
                                      break;
          case STRATUM_SUCCESS:       ESP_LOGD(TAG, "  Parsed JSON: Success"); break;
          default:                    ESP_LOGD(TAG, "  Parsed JSON: Unknown"); break;
      }
    }
...
void runMonitor(void *name) {
  ...
  while (1) {
    if ((frame % REDRAW_EVERY) == 0) {
      ...
      if (elapsedKHs == 0) {
        ESP_LOGD(TAG, ">>> [MONITOR] Miner: newJob>%s / inRun>%s) - Client: connected>%s / subscribed>%s / wificonnected>%s",
          YESNO(mMiner.newJob), YESNO(mMiner.inRun),
          YESNO(client.connected()), YESNO(isMinerSuscribed),
