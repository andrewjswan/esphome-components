...
void sha256_ll_init(void) { ESP_LOGD(TAG, "[SHA-LL] Hardware SHA-256 initialized"); }
...
static inline bool IRAM_ATTR ll_read_digest_if(uint8_t *hash_out) {
  ...
  if (debug_log) {
    ESP_LOGD(TAG, "[DEBUG] first=%08x H[31]=%02x H[30]=%02x H[29]=%02x H[28]=%02x", first, hash_out[31], hash_out[30],
             hash_out[29], hash_out[28]);
  }
