...
#ifndef bswap_32
#define bswap_32(a) ((((uint32_t) (a) << 24) & 0xff000000) | (((uint32_t) (a) << 8) & 0xff0000) | (((uint32_t) (a) >> 8) & 0xff00) | (((uint32_t) (a) >> 24) & 0xff))
#endif
...
void swap_endian_words(const char * hex_words, uint8_t * output) {
  ...
  if (hex_length % 8 != 0) {
    ESP_LOGE(TAG, "SWAP Endian: Must be 4-byte word aligned");
    return;
  }
...
*/
  ...
  for (size_t i = 76; i < 76+4; i++)
    ESP_LOGD(TAG, "%02x", mMiner.bytearray_blockheader[i]);
  ESP_LOGD(TAG, "bytearray_blockheader: %s", esphome::format_hex_pretty(mMiner.bytearray_blockheader, str_len).c_str());
  #endif
  return mMiner;
}
